...
public class SetTerrainObstacles : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (TreeInstance tree in Obstacle)
        {
            CapsuleCollider collider = terrain.terrainData.treePrototypes[tree.prototypeIndex].prefab.GetComponent<Collider>() as CapsuleCollider;
            if (collider == null) continue;
            Vector3 localpos = new Vector3(tree.position.x * width,tree.position.y * hight,tree.position.z * lenght);
